...
public class KhachhangDAL {
	...
	public KhachhangDAL(Context context) {
	...
	/**
	 * Ham luu danh sach Khach hang vao DB
	 * @param context
	 * @param url
	 */
	public static void saveToDB(Context context, String url) {
		try {
			...
			if (glstData.size() > 0) {
				...
				KhachhangDAL dal = new KhachhangDAL(context);
				for (TenKH item : glstData) {

					// Kiem tra Khach hang da ton tai hay chua?
					if (dal.getById(item.getId()) == null) {
						// Save KhachHang to DB
						long resultInsert = dal.add(item);
						Log.d("INSERT-KHACHHANG-ITEM", "" + resultInsert);
					} else {
						Log.d("KHACHHANG-ITEM-EXISIT", "ID: " + item.getId() + " is exist");
					}
				}
			}
		} catch (Exception e) {
	...
	public long add(TenKH data) { 
		...
		try {
			...
			if (database != null) {
				...
				result = database.insert(TenKH.TABLE_NAME, null, values);
			}
			closeDB();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int delete(String id) {
		...
		try {
			...
			if (database != null) {
				...
				result = database.delete(TenKH.TABLE_NAME, where, new String[] { String.valueOf(id) });
			}
			closeDB();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public TenKH getById(String id) {
		...
		try {
			...
			if (db != null) {
				...
				Cursor cursor = db.query(TenKH.TABLE_NAME,
	...
	public ArrayList<TenKH> getAll() {
		ArrayList<TenKH> glstKhachHang = new ArrayList<TenKH>();
		try {
			openDB();
			if (database != null) {
				Cursor cursor = null;
				String selectQuery = "SELECT * FROM " + TenKH.TABLE_NAME;
