...
public class TrangThaiDonHangDAL {
	...
	public TrangThaiDonHangDAL(Context context) {
	...
	/**
	 * Ham luu danh sach Trang thai don hang vao DB
	 * @param context
	 * @param url
	 */
	public static void saveToDB(Context context, String url) {
		try {
			...
			if (glstData.size() > 0) {
				...
				TrangThaiDonHangDAL dal = new TrangThaiDonHangDAL(context);
				for (TenKH item : glstData) {

					if (dal.getById(item.getId()) == null) {
						long resultInsert = dal.add(item);
						Log.d("INSERT-TRANG-THAI-DON-HANG-ITEM", "" + resultInsert);
					} else {
						Log.d("TRANG-THAI-DON-HANG-ITEM-EXISIT", "ID: " + item.getId() + " is exist");
					}
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Them moi 1 Trang thai don hang vao DB
	 * @param context
	 * @param data
	 * @return
	 */
	public long add(TenKH data) { 
		...
		try {
			...
			if (database != null) {
				...
				result = database.insert(TenKH.TABLE_NAME_TTDH, null, values);
			}
			closeDB();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public int delete(String id) {
		...
		try {
			...
			if (database != null) {
				...
				result = database.delete(TenKH.TABLE_NAME_TTDH, where, new String[] { String.valueOf(id) });
			}
			closeDB();
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * Ham lay Trang thai don hang qua id
	 * @param id
	 * @return
	 */
	public TenKH getById(String id) {
		...
		try {
			...
			if (db != null) {
				...
				Cursor cursor = db.query(TenKH.TABLE_NAME_TTDH,
	...
	/**
	 * Lay tat ca Trang thai don hang offline co trong CSDL
	 * @return
	 */
	public ArrayList<TenKH> getAll() {
		...
		try {
			...
			if (database != null) {
				Cursor cursor = null;
				String selectQuery = "SELECT * FROM " + TenKH.TABLE_NAME_TTDH;
